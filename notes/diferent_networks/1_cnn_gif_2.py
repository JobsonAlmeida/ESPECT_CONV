...
for caminho, titulo in zip(caminhos_imagens, titulos):
    ...
    # Define uma fonte padrão do sistema (pode ajustar o tamanho alterando o número 24)
    # Se quiser uma fonte específica ex: "arial.ttf", use ImageFont.truetype("arial.ttf", 24)
    try:
        font = ImageFont.truetype("arial.ttf", 24)
    except IOError:
        font = ImageFont.load_default() # Fallback caso não ache a Arial no sistema
        
    # Desenha o texto na imagem (Posição X=20, Y=20, Cor=Preto)
    # Dica: mude fill=(255, 255, 255) para Branco se o fundo do seu gráfico for escuro
    draw.text((20, 20), titulo, fill=(0, 0, 0), font=font)
